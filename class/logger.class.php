...
class logger {
				...
				if ($this->status == "clu" || $this->status == "gh") {
						if (preg_match("/\\[(\d{2}):(\d{2}):(\d{2})\\]/", $this->log_, $time)) {
							  if(!$this->log_time) {
								...
								$this->log_ = "<div class='row'>$this->log_";
						} else {
							  $this->log_time = false;
    ...
    private function type() {
		    if (preg_match("/issued server command:/", $this->log_)) {
				...
				$this->log_ = preg_replace("/\\[JDA MainWS-ReadThread\\/INFO\\]: \\[DiscordSRV\\] Chat:/", "<div style='color: #00edff; padding: 0px 5px 0px 0px;'>[Discord]</div>", $this->log_);
